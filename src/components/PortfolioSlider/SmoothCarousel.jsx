...
@keyframes card-slide-in-preview {
    0% {
...
/* Custom CSS classes for applying animations */
.card-expand-animation {
    animation: card-expand-from-preview 1s cubic-bezier(0.4, 0, 0.2, 1) forwards;
}

.card-shrink-animation {
    animation: card-shrink-exit 1s cubic-bezier(0.4, 0, 0.2, 1) forwards;
}

.card-slide-in-animation {
    animation: card-slide-in-preview 1s cubic-bezier(0.4, 0, 0.2, 1) forwards;
}
...
const SmoothCarousel = ({images}) => {
  ...
  return (
    <>
      ...
      <div className="relative w-full max-w-5xl mx-auto h-[500px] bg-gradient-to-br from-blue-50/5 to-blue-100/10 rounded-2xl shadow-xl">
        {/* Background Pattern */}
        <div className="absolute inset-0 opacity-30">
          <div className="absolute inset-0 bg-gradient-to-br from-blue-50/5 via-transparent to-purple-50/5"></div>
        </div>
        ...
        <div className="relative h-full flex items-center justify-center p-8 overflow-hidden">
          ...
          {/* Shrinking Card - The one going to left */}
          {isAnimating && (
            <div className="absolute left-0 lg:w-[69%] h-full card-shrink-animation z-10 md:w-[100%]">
              <div className="relative h-full bg-white rounded-xl overflow-hidden shadow-2xl">
                <div className="relative h-full overflow-hidden">
                  <img 
                    src={images[previousIndex].image} 
          ...
          {/* Active Card - Expanding from preview position */}
          <div className={`absolute lg:w-[69%] h-full z-30 left-0 md:w-[100%] ${
            isAnimating ? 'card-expand-animation' : ''
          }`}>
            <div className="relative h-full bg-white rounded-xl overflow-hidden shadow-2xl group">
              <div className="absolute inset-0 bg-gradient-to-r from-blue-200/20 to-purple-200/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          ...
          {/* Preview Card - Will slide from right */}
          <div className={`absolute right-1 lg:w-[30%] h-[100%] z-20 md:w-[100%] ${
            isAnimating ? 'card-slide-in-animation' : ''
          }`}>
            <div className="relative h-full bg-white/80 backdrop-blur-sm rounded-xl overflow-hidden shadow-lg opacity-60 hover:opacity-80 transition-opacity duration-300">
              <div className="relative h-full overflow-hidden">
                <img 
          ...
          {/* Navigation Buttons */}
          <button
            className="absolute left-4 top-1/2 z-50 -translate-y-1/2 w-12 h-12 rounded-full bg-white/80 backdrop-blur-sm border-2 border-gray-200 text-gray-800 hover:bg-green-800 hover:text-white transition-all duration-300 flex items-center justify-center shadow-md"
            onClick={prevSlide}
            disabled={isAnimating}
          >
            <ChevronLeftIcon className="h-6 w-6" />
          ...
          <button
            className="absolute right-4 top-1/2 z-50 -translate-y-1/2 w-12 h-12 rounded-full bg-white/80 backdrop-blur-sm border-2 border-gray-200 text-gray-800 hover:bg-green-800 hover:text-white transition-all duration-300 flex items-center justify-center shadow-md"
            onClick={nextSlide}
            disabled={isAnimating}
          >
            <ChevronRightIcon className="h-6 w-6" />
        ...
        {/* Indicators */}
        <div className="absolute -bottom-12 left-1/2 z-40 -translate-x-1/2 flex space-x-2">
          {images.map((_, index) => (
            <button
              key={index}
